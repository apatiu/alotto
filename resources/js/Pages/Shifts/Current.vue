<template>
    <app-layout>
        ...
        <Card class="shift-summary mt-10">
            <template #title>
                สรุปกะทำงาน
            </template>
            <template #content>
...
export default {
    ...
    watch: {
        ...
        closing(val) {
            if (val) {
                this.formShift.next_d = moment(this.shift.d).add(1, 'd').toDate();
            }
        }
    },
    mounted() {
        this.formShift.d = new Date(this.shift.d)
    },
    computed: {
        shiftClosed() {
    ...
    methods: {
        ...
        closeShift() {
            ...
            this.formShift.post(route('shifts.close', this.shift.id));
        }
    },
}
</script>
